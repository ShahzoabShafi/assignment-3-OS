...
struct PCB *run_pcb_to_completion(struct PCB *pcb) {
    while (pcb_has_next_instruction(pcb)) {
        ...
        if (program_get_frame_for_page(pcb->program, pcb_current_page(pcb)) < 0) {
            if (program_load_page_on_fault(pcb->program, pcb_current_page(pcb)) != 0) {
                free_pcb(pcb);
                return NULL;
            }
            return pcb;
        ...
        instruction = pcb_current_instruction(pcb, &frame_index);
        frame_store_touch(frame_index);
...
static void reset_script_state(void) {
...
int my_exec(char *args[], int args_size, bool MT) {
    ...
    for (int n = 0; n < args_size; ++n) {
        bool created = false;
        struct Program *program = get_or_create_program(args[n], &created);
        struct PCB *pcb = NULL;
        ...
        if (created && program_load_initial_pages(program, 2) != 0) {
            printf("Failed to load program pages\n");
            goto cleanup;
        }

        pcb = create_process(program);
        if (!pcb) {
            printf("Failed to create process\n");
            goto cleanup;
        }
